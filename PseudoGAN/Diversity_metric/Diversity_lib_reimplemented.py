...
'''
def control_query_data(bias, isGenData , imagenet, class_idx, gen_idx=0, controlSize=50):
    m=int(controlSize/2)
    ...
    #Queried data--------------------------------------------------------------------------------------
    if isGenData==True:
        data=torch.load("../../Real_GAN/Generated_Data/StyleGANv2_celebaHQ_to_publish/Amazon_mturk/np/generated_images_1k_64x64.pt").numpy()
        #Dummy labels
    ...
    for i in tqdm(range(len(names))):
        ...
        if imagenet==True:
            ...
            __,__,imageToFeatures[names[i]] = feat_extractor(image) #logits, probas, feat
